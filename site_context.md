...
## This is the tree of myproject’s root folder

```sh

├── backend
│   ├── backend@1.0.0
│   ├── blocked_news_words.txt
│   ├── dist
│   │   ├── db.js
│   │   ├── db.js.map
│   │   ├── index.js
...
│   ├── simple_setup.sh
│   ├── sql
│   │   ├── 000_create_admin_user.sql
│   │   ├── 010_setup_database.sql
│   │   ├── 020_create_quotes_table.sql
│   │   ├── 030_course_tables.sql
│   │   ├── 040_todo_table.sql
│   │   ├── csv
│   │   │   ├── course_data.csv
│   │   │   ├── course_names.csv
...
│   └── tsconfig.test.json
├── docker-compose.yml
├── DONOTUSE-docker-compose.prod.yml
├── endpoints.csv
├── frontend
│   ├── components
│   │   ├── admin
...
## This is the software install in the backend 
```json
{
  "name": "backend",
  "version": "1.0.0",
  ...
  "packages": {
    ...
    "node_modules/@types/express-validator": {
      "version": "3.0.2",
      "resolved": "https://registry.npmjs.org/@types/express-validator/-/express-validator-3.0.2.tgz",
      "integrity": "sha512-dV+1u6absDDEIVe5jd5ID1XYvPPvOSvggMWh09VYM3TeBjAjBDtwJWyJg/5PnK7s8FRgbOtHgD6oX3N2MKWwDw==",
...
```
# This is the woftware installed on the frontend
...
CMD ["./start.sh"]
```
## This is the main utuility to build/rebuild the site located at ./ REBUILD
```sh
#!/bin/bash
#
...
#
# USAGE:
#   utils/REBUILD [--all] [--frontend] [--backend] [--nginx] [--postgres] [--wipe] [--restore]
#
# OPTIONS:
...
#   --postgres          Rebuild and restart only the PostgreSQL database
...
#   utils/REBUILD --postgres --restore # Rebuild PostgreSQL and restore from backup
...
# # Delete the named volume (actual project‑prefixed name is shown by `docker volume ls`)
...
# docker compose up -d db
    ...
    if [ -n "$RESTORE" ] && [ "$RESTORE" = "--restore" ]; then
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        *)
            ...
            echo "Usage: ./REBUILD [--all] [--frontend] [--backend] [--nginx] [--postgres] [--wipe] [--restore]"
            exit 1
            ;;
    esac
done
