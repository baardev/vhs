    ...
    const fetchCourses = async () => {
      try {
        ...
        if (Array.isArray(response.data)) {
          ...
          setCourses([]);
          setError(dict.coursesPage?.invalidDataFormat || 'Invalid data format received from server.');
        }
      } catch (err) {
        console.error('Error fetching courses:', err);
        setError(dict.coursesPage?.fetchError || 'Failed to load courses. Please try again later.');
      } finally {
    ...
    <div className="min-h-screen bg-[#f8f9fa] dark:bg-[#111] py-12 px-4 sm:px-6 lg:px-8 relative">
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-3xl font-bold text-[#2d6a4f] dark:text-[#4fd1c5]">
            {dict.coursesPage.title || 'Golf Course Database'}
          </h1>
          {isAuthenticated ? (
            <Link
            ...
            >
              {dict.coursesPage.addNewCourse || 'Add New Course'}
            </Link>
          ) : (
            <Link
              ...
              className="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-[#2d6a4f] hover:bg-[#1b4332] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#40916c] dark:bg-[#4fd1c5] dark:hover:bg-[#38b2ac] dark:focus:ring-[#38b2ac]"
            >
              {dict.coursesPage.loginToAddCourses || 'Log in to add new courses'}
            </Link>
          )}
        </div>
        ...
        <div className="bg-white dark:bg-[#1a2b41] p-6 rounded-lg mb-8">
          <p className="text-gray-700 dark:text-[#b5ceff] text-lg">
            {dict.coursesPage.description || 'Browse our comprehensive database of golf courses. Click on a card to view detailed information including tee options, course ratings, and hole-by-hole data.'}
          </p>
        </div>

        <div className="p-2 mb-4 bg-gray-100 dark:bg-gray-800 rounded text-xs">
          {dict.coursesPage?.debug ? 
            `${dict.coursesPage.debug}: ${pageRenders} | ${dict.coursesPage.dataLoaded || 'API data loaded'}: ${courses.length} ${dict.coursesPage.coursesLoaded || 'courses'} | ${dict.coursesPage.loading || 'Loading'}: ${isLoading ? dict.coursesPage.yes || 'Yes' : dict.coursesPage.no || 'No'}` : 
